...
exports.regUser = asyncHandler(async (req, res, next) => {
  ...
  const user = new User(name, email, "user", password);
  try {
    await user.save();
    ...
    const confirmUrl = `${req.protocol}://${req.get("host")}/api/v1/auth/confirmuser/${token}`;
    console.log(confirmUrl);
...
exports.sendResetToken = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorResponse("This email is not registered in our database", 401),
    );
  }
